...
def generate_response(query_text, vectorstore, callback):
    ...
    rag_prompt = [
        SystemMessage(
            content="너는 문서에 대해 질의응답을 하는 '씨엔이'야. 주어진 문서를 참고하여 사용자의 질문에 답변을 해줘. 문서에 내용이 정확하게 나와있지 않으면 최대한 너의 지식을 활용해서 답변해줘. 이모티콘을 사용해서 친절하게 대답해줘"),
            HumanMessage(content=f"질문:{query_text}\n\n{docs}"),
    ]
...
# page title
st.set_page_config(page_title='🦜🔗 문서 기반 요약 및 QA 챗봇2')
st.title('🦜🔗 문서 기반 요약 및 QA 챗봇2')
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [
        ChatMessage(
            role="assistant", content="안녕하세요! 저는 문서에 대한 이해를 도와주는 LG CNS 챗봇입니다. 어떤게 궁금하신가요?"
        )
    ]
